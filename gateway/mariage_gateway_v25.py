...
class V25Handler(v24.V24Handler):
    server_version = "MariageGateway/2.5"
    ...
    def do_POST(self):
        ...
        with QUIZ_LOCK:
            ...
            if route == "/api/v25/quiz/config":
                ...
                quiz["questions"] = cleaned; quiz["current"] = -1
            ...
            if route == "/api/v25/quiz/control":
                ...
                if action == "launch":
                    ...
                    quiz["current"] = index; quiz["questions"][index]["status"] = "open"
                elif action in {"close", "reveal"}:
                    ...
                    for q in quiz["questions"]:
                        q["status"] = "idle"; q["responses"] = {}
                    quiz["current"] = -1; quiz["adjustments"] = {table: 0 for table in TABLES}
                else:
...
def start_v25(self):
    ...
    try:
        ...
        self.status.config(text="● Passerelle V2.5 active — port 8788", fg="#74d3ae")
        gateway.log("Passerelle mariage V2.5 démarrée sur 127.0.0.1:8788")
