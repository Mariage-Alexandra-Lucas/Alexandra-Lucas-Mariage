(() => {
  ...
  function controlPanel(){
    ...
    const nextIndex=Math.min(14,Math.max(0,(state.quiz.currentIndex??-1)+1));
    return `<section class="card quiz-control"><div class="section-title">Console DJ</div><h2>Animation Elle ou Lui</h2><div class="control-buttons"><button class="btn launch-quiz" data-quiz-control="launch">Lancer le quiz</button><button class="btn danger" data-quiz-control="reset">Recommencer le quiz</button></div><select class="field" id="quiz-question-select">${options}</select><div class="control-buttons"><button class="btn" data-quiz-control="start">${q?'Afficher la question sélectionnée':'Lancer la question 1'}</button><button class="btn secondary" data-quiz-control="close" ${!q||q.status!=='open'?'disabled':''}>Clôturer les votes</button><button class="btn secondary" data-quiz-control="reveal" ${!q||q.status!=='closed'?'disabled':''}>Afficher la bonne réponse</button><button class="btn" data-quiz-control="next" data-next-index="${nextIndex}" ${!q||q.status!=='revealed'||state.quiz.currentIndex>=14?'disabled':''}>Question suivante</button></div>${q?`<div class="dj-question"><span>Question ${q.number}/15 — ${quizEsc(q.status)}</span><h3>${quizEsc(q.text)}</h3><form id="correct-answer-form"><label>Bonne réponse</label><div class="answer-correction"><select class="field" name="answer"><option value="elle" ${q.correctAnswer==='elle'?'selected':''}>Elle</option><option value="lui" ${q.correctAnswer==='lui'?'selected':''}>Lui</option></select><button class="btn secondary">Corriger</button></div></form></div><p class="vote-progress">${Object.keys(responses).length}/4 tables ont répondu</p>`:'<p>Le jeu est prêt. Cliquez sur « Lancer le quiz » : la question 1 s’ouvrira automatiquement sur tous les téléphones.</p>'}${scoreBoard(false)}</section>`;
  }
  ...
  async function quizControl(action,button){
    let index=Number(document.querySelector('#quiz-question-select')?.value||0);
    if(action==='next')index=Number(button?.dataset.nextIndex??Math.min(14,state.quiz.currentIndex+1));
    if(action==='reset'&&!confirm('Recommencer entièrement le quiz ? Les votes et les scores seront effacés et la question 1 sera remise en attente.'))return;
    state.quiz=await api('/api/v25/quiz/control',{method:'POST',body:JSON.stringify({action,index})});render();
    if(action==='reset')toast('Quiz réinitialisé');
  }
  function bindQuiz(){
    ...
    app.querySelectorAll('[data-quiz-control]').forEach(button=>button.onclick=()=>quizControl(button.dataset.quizControl,button).catch(e=>toast(e.message)));
  ...
  function startPolling(){clearInterval(quizTimer);quizSignature=JSON.stringify(state.quiz);quizTimer=setInterval(async()=>{if(document.hidden)return;const next=await api('/api/v25/quiz').catch(()=>null);if(next&&JSON.stringify(next)!==quizSignature){state.quiz=next;quizSignature=JSON.stringify(next);render()}},1000)}
  shell=function(){
